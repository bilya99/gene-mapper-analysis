...
X = 4  # Replace with your X value
Y = 8  # Replace with your Y value
...
# Assuming you already have your dataframes: df_exported_table and df_sample_order
# If you need to read them from files, uncomment and modify the lines below:
# df_exported_table = pd.read_excel('exported_table.xlsx')
# df_sample_order = pd.read_excel('sample_order.xlsx')

# Create a mapping dictionary from df_sample_order
# name_mapping = dict(zip(df_sample_order['Default_name'], df_sample_order['Sample_number']))

# # Replace values in "Sample File Name" column using the mapping
# df_exported_table['Sample File Name'] = df_exported_table['Sample File Name'].map(name_mapping).fillna(df_exported_table['Sample File Name'])
